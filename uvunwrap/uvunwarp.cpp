...
namespace LFX {
    ...
    void Unwrap(NLOutMesh& out, const NLInMesh* mesh)
	{
        ...
        for (const auto& v : mesh->vertices) {
            imesh.add_vertex(vec3(v.x, v.y, v.z), vec2(0, 0));
        }
        for (const auto& f : mesh->triangles) {
            imesh.begin_facet();
            imesh.add_vertex_to_facet(f.x);
            imesh.add_vertex_to_facet(f.y);
            imesh.add_vertex_to_facet(f.z);
            imesh.end_facet();
        }
        ...
        out.uvs.resize(imesh.nb_vertices());
        assert(out.uvs.size() == mesh->vertices.size());
        for (size_t i = 0; i < imesh.nb_vertices(); ++i) {
            Float2& v = out.uvs.at(i);
            v.x = (float)imesh.vertex[i].tex_coord.x;
            v.y = (float)imesh.vertex[i].tex_coord.y;
        }
